...
target = r'<div \s*className="h-36 rounded-lg bg-slate-950 border border-slate-800 relative cursor-pointer group flex items-center justify-center overflow-hidden"\s*onClick=\{\(\) => setShowImageLightbox\(true\)\}\s*title="Klicken für interaktive Vollbild-Vorschau"\s*>\s*<canvas ref=\{vectorOverlayCanvasRef\} className="max-w-full max-h-full object-contain z-10" />\s*<img src=\{rasterImage\.src\} className="absolute max-w-full max-h-full object-contain opacity-20 pointer-events-none" alt="" />\s*<div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity z-20 backdrop-blur-sm">\s*<div className="flex items-center gap-2 text-white bg-indigo-600/80 px-4 py-2 rounded-full font-bold shadow-lg">\s*<Expand className="w-4 h-4" /> Großansicht öffnen\s*</div>\s*</div>\s*</div>\s*</>\s*\) : \('

replacement = '''<div className="h-36 rounded-lg bg-slate-950 border border-slate-800 relative cursor-pointer group flex items-center justify-center overflow-hidden" onClick={() => setShowImageLightbox(true)} title="Klicken für interaktive Vollbild-Vorschau">
  ...
  {rasterOpacityBW > 0 && bwDataUrl && <img src={bwDataUrl} className="absolute max-w-full max-h-full object-contain pointer-events-none mix-blend-screen" style={{ opacity: rasterOpacityBW / 100 }} alt="" />}
  ...
  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity z-20 backdrop-blur-sm">
    <div className="flex items-center gap-2 text-white bg-indigo-600/80 px-4 py-2 rounded-full font-bold shadow-lg"><Expand className="w-4 h-4" /> Großansicht öffnen</div>
  </div>
</div>
