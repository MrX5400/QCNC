...
# 1. State changes
idx_state = full_text.find("const [liveDragOffsets, setLiveDragOffsets] = useState<{ [id: string]: { x: number; y: number } }>({});")
state_block = """  const [liveDragOffsets, setLiveDragOffsets] = useState<{ [id: string]: { x: number; y: number } }>({});
...
"""
full_text = full_text.replace("const [liveDragOffsets, setLiveDragOffsets] = useState<{ [id: string]: { x: number; y: number } }>({});", state_block)

# 2. Add handleGenerateGcode
idx_gen = full_text.find("const handleDownloadGcode")
handle_fn = """
...
"""
full_text = full_text[:idx_gen] + handle_fn + full_text[idx_gen:]

# 3. Replace useMemo hooks
memoregs = [
...
# Insert fallback variable declarations
fallback_vars = """
...
# 6. Add button UI before the bottom action bar
ui_target = """              )}
            </div>
          </div>
        </div>
