...
# Find the threshold block using regex
match = re.search(r'(<input type="range" min="0" max="255" value=\{settings\.threshold\}.*?</div>)', text, flags=re.DOTALL)
if match:
    text = text[:match.end()] + "\n" + adjustments + text[match.end():]
    print("SUCCESSFULLY INSERTED BILDANPASSUNGEN")
else:
